...
class Database
{
    ...
    */
    ...
    {
        $stmt = $this->executeFetchAll("SELECT password FROM Users WHERE name='$user'")[0];
    ...
    {
        $stmt = $this->executeFetchAll("SELECT name FROM Users WHERE name='$user'");
        if ($stmt) {
    ...
    {
        $this->stmt = $this->pdo->prepare("INSERT INTO Users (name, password, role) VALUES ('$user', '$pass', '$role')");
        $this->stmt->execute();
    }
    ...
    {
        $this->stmt = $this->pdo->prepare("UPDATE Users SET password='$pass' WHERE name='$user'");
        $this->stmt->execute();
    }



    public function updateUser($user, $pass, $new_user, $role)
    {
        if ($pass) {
            $crypt_pass = password_hash($pass, PASSWORD_DEFAULT);
            $this->stmt = $this->pdo->prepare("UPDATE Users SET name='$new_user', password='$crypt_pass', role='$role' WHERE name='$user'");
        } else {
            $this->stmt = $this->pdo->prepare("UPDATE Users SET name='$new_user', role='$role' WHERE name='$user'");
        }
        $this->stmt->execute();
    }
    ...
    {
        $this->stmt = $this->pdo->prepare("DELETE FROM Users WHERE name='$user'");
        $this->stmt->execute();
    }
}
